defmodule SlimOverflowWeb.CategoryView do
  ...
  def time_posted(%Category{} = category) do
    Categories.time_posted(category) |> SlimOverflowWeb.ViewHelpers.format_time()
  end

  def time_updated(%Category{} = category) do
    Categories.time_updated(category) |> SlimOverflowWeb.ViewHelpers.format_time()
  end
  ...
  def sort(categories, sort_by) do
    case sort_by do
      ...
      "date" -> Enum.sort_by(categories, &Categories.time_posted()/1, &SlimOverflowWeb.ViewHelpers.naive_date_time_compare()/2) |> Enum.reverse()
