defmodule PlungerWeb.AuthTest do
  use PlungerWeb.ConnCase

  alias Plunger.Questions
  alias PlungerWeb.Auth
  alias Plunger.Repo

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(PlungerWeb.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Plunger.Accounts.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Plunger.Accounts.User{id: 123})
      |> send_resp(:ok, "")
